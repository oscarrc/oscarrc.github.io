...
const Terminal = ({ isOpen, setOpen, toggleTheme }) => {
    ...
    return (
        <div ref={termRef} onClick={ () => inputRef.current.focus() } className={`z-50 mockup-code fixed mx-4 sm:mx-16 my-16 pt-12 transition-all inset-0 duration-250 ease-in-out origin-bottom-right ${isOpen ? 'scale-1 translate-y-0 translate-x-0' : 'scale-0 translate-y-8 translate-x-8'} shadow-lg`}>
